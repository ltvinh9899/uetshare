...
public interface ReactIconQuestionRepositoryInterface extends JpaRepository<ReactIconQuestion, Integer> {
    ...
    @Query(value = "select * from react_icon_question where react_icon_question.question_id = ?1 limit 0, ?2", nativeQuery = true)
...
//    List<ReactIconQuestion> getReactIconByCommentId(Long comment_id, Integer index);
    ...
    @Query(value = "select * from react_icon_question where react_icon_question.id = ?1", nativeQuery = true)
    ReactIconQuestion getReactIconQuestionById(Long id);

    @Modifying
    @Transactional
    @Query(value = "delete from react_icon_question where react_icon_question.id = ?1", nativeQuery = true)
    void deleteByReactIconQuestionId(Long id);
}
